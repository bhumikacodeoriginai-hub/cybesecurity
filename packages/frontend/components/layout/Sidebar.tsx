...
export default function Sidebar() {
  ...
  return (
    <aside className="fixed left-0 top-0 h-full w-64 bg-dark-950 border-r border-dark-700/50 z-40 flex flex-col">
      {/* Logo */}
      <div className="h-16 flex items-center px-6 border-b border-dark-700/50">
        <Link href="/courses" className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className="text-[10px] text-dark-500 -mt-0.5 tracking-wider uppercase">Academy</p>
          </div>
        </Link>
      </div>
      ...
      <nav className="flex-1 px-3 py-6 space-y-1">
        <p className="px-4 text-[10px] font-semibold text-dark-500 uppercase tracking-widest mb-3">Learn</p>
        {navItems.map((item) => {
      ...
      <div className="p-4 border-t border-dark-700/50">
        <div className="flex items-center gap-3 px-2">
          <div className="w-8 h-8 bg-cyber-400/10 border border-cyber-400/20 rounded-full flex items-center justify-center">
            <svg className="w-4 h-4 text-cyber-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M13 10V3L4 14h7v7l9-11h-7z" />
            </svg>
          </div>
          <div className="flex-1 min-w-0">
            <p className="text-xs font-medium text-dark-300">CyberSec Academy</p>
            <p className="text-[10px] text-dark-500">v2.0 — 10 Modules, 72 Lessons</p>
          </div>
        </div>
      </div>
    </aside>
